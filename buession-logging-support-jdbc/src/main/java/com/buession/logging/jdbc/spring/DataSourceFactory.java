...
/**
 * 数据源 {@link  com.buession.jdbc.datasource.DataSource} 工厂
 *
 * @author Yong.Teng
 * @since 3.0.0
 */
public class DataSourceFactory {
	...
	private final static Map<String, Class<? extends DataSource<?, ?>>> DATA_SOURCE_MAP =
			new LinkedHashMap<>(5);
	...
	protected DataSource<?, ?> createDataSource() {
		...
		final Class<? extends DataSource<?, ?>> dataSourceClazz = getDataSourceType();
		try{
			...
			this.dataSource = BeanUtils.instantiateClass(constructor, driverClassName, url, username, password);

			dataSource.setLoginTimeout(getLoginTimeout());

			this.dataSource.setInitSQL(getInitSQL());

			this.dataSource.setConnectionProperties(getConnectionProperties());

			if(getDataSourceConfiguration() != null){
				this.dataSource.setQueryTimeout(getDataSourceConfiguration().getQueryTimeout());
				this.dataSource.setDefaultTransactionIsolation(
						getDataSourceConfiguration().getDefaultTransactionIsolation());
				this.dataSource.setDefaultAutoCommit(getDataSourceConfiguration().getDefaultAutoCommit());
				this.dataSource.setAccessToUnderlyingConnectionAllowed(
