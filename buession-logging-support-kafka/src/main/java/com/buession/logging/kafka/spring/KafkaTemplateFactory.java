 ...
 * @since 0.0.1
 */
class KafkaTemplateFactory<K, V> {

	private final ProducerFactory<K, V> producerFactory;

	KafkaTemplateFactory(final ProducerFactory<K, V> producerFactory){
		this.producerFactory = producerFactory;
	}

	protected KafkaTemplate<K, V> createKafkaTemplate(){
		final KafkaTemplate<K, V> kafkaTemplate = new KafkaTemplate<>(producerFactory.createProducerFactory());
