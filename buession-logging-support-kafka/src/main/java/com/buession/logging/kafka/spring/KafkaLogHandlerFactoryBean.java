...
public class KafkaLogHandlerFactoryBean extends BaseLogHandlerFactoryBean<KafkaLogHandler> {
	...
	public final static List<String> DEFAULT_BOOTSTRAP_SERVERS = new ArrayList<>(ListBuilder.of("localhost:9092"));
	...
	public void setProperties(Map<String, String> properties) {
		if(Validate.isNotEmpty(properties)){
	...
	public void afterPropertiesSet() throws Exception {
		Assert.isEmpty(bootstrapServers, "Kafka bootstrap servers cloud not be null.");

		final ProducerFactory<?, Object> producerFactory = new ProducerFactory<>(buildProducerProperties(),
				transactionIdPrefix);
		final KafkaTemplateFactory<?, Object> kafkaTemplateFactory = new KafkaTemplateFactory<>(producerFactory);
		final KafkaTemplate<?, Object> kafkaTemplate = kafkaTemplateFactory.createKafkaTemplate();
	...
	protected Properties buildProducerProperties() {
		...
		propertyMapper.from(bootstrapServers).to(properties.in(CommonClientConfigs.BOOTSTRAP_SERVERS_CONFIG));
		propertyMapper.from(bootstrapServers).to(properties.in(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG));
		propertyMapper.from(clientId).to(properties.in(CommonClientConfigs.CLIENT_ID_CONFIG));
		...
		propertyMapper.from(StringSerializer::new).to(properties.in(ProducerConfig.KEY_SERIALIZER_CLASS_CONFIG));
		propertyMapper.from(new JsonSerializer<>()).to(properties.in(ProducerConfig.VALUE_SERIALIZER_CLASS_CONFIG));
