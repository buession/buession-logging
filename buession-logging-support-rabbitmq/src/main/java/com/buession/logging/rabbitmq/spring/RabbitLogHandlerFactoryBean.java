...
public class RabbitLogHandlerFactoryBean extends BaseLogHandlerFactoryBean<RabbitLogHandler> {
	...
	public void afterPropertiesSet() throws Exception {
		Assert.isBlank(host, "RabbitMq host cloud not be blank, empty or null.");
		Assert.isBlank(virtualHost, "VirtualHost host cloud not be blank, empty or null.");

		final ConnectionFactory connectionFactory = new ConnectionFactory(host, port, username, password, virtualHost,
				connectionTimeout, sslConfiguration, requestedHeartbeat, requestedChannelMax, publisherReturns,
				publisherConfirmType, cache);
		final RabbitTemplateFactory rabbitTemplateFactory = new RabbitTemplateFactory(connectionFactory, template,
				publisherReturns);
		final RabbitTemplate rabbitTemplate = rabbitTemplateFactory.createRabbitTemplate();
